...
func NewApp(ctx context.Context, conf Config) (*ApplicationContext, error) {
	...
	formatWriter := export.NewFixedLengthFormatter(userType)
	writer, err := export.NewFileWriter(GenerateFileName)
	exportService := export.NewExporter(db, userType, BuildQuery, formatWriter.Format, writer.Write, writer.Close)
...
func BuildQuery(ctx context.Context) (string, []interface{}) {
	query := "select id, username, email, phone, status, createdDate from users1;"
